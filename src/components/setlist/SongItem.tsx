    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <button
          {...attributes}
          {...listeners}
          className="text-zinc-600 hover:text-zinc-400 cursor-grab active:cursor-grabbing touch-none flex-none"
        >⣿</button>
        ...
        <div className="flex-1 min-w-0" onClick={() => setExpanded(e => !e)}>
          ...
          <div className="text-xs text-zinc-400 truncate">
            {item.artist}
            {item.tom ? <span className="ml-1 text-blue-400">[{item.tom}]</span> : ''}
        ...
        <div className="flex items-center gap-1 flex-none">
          {onPreview && (
            <button
              ...
              className="text-zinc-400 hover:text-amber-400 px-1 text-lg"
              title="Ver cifra"
            >👁</button>
          )}
          <button
            onClick={() => setExpanded(e => !e)}
            className="text-zinc-400 hover:text-white px-1 text-sm"
          >{expanded ? '▲' : '▼'}</button>
          <button
            onClick={() => removeItem(item.blockIndex, item.itemIndex)}
            className="text-zinc-600 hover:text-red-400 px-1 text-lg leading-none"
          >×</button>
        </div>
      </div>
      ...
      {/* Painel expandido */}
      {expanded && (
        <div className="border-t border-zinc-700 px-3 py-3 space-y-2 bg-zinc-900">
          <div className="grid grid-cols-2 gap-2">
            <label className="text-xs text-zinc-500">
              ...
              <select
                ...
                className="mt-1 w-full bg-zinc-800 text-white rounded px-2 py-1 text-sm border border-zinc-700"
              >
                <option value="">—</option>
          ...
          <label className="block text-xs text-zinc-500">
            ...
            <input
              type="text"
              value={item.preparacao}
              onChange={e => update({ preparacao: e.target.value })}
              className="mt-1 w-full bg-zinc-800 text-white rounded px-2 py-1 text-sm border border-zinc-700"
          ...
          <label className="block text-xs text-zinc-500">
            ...
            <input
              type="text"
              value={item.cifraDriveId}
              onChange={e => update({ cifraDriveId: e.target.value })}
              className="mt-1 w-full bg-zinc-800 text-white rounded px-2 py-1 text-xs border border-zinc-700 font-mono"

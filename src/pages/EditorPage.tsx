  ...
  return (
    <div className="min-h-screen bg-black text-white pb-24">
      ...
      <div className="sticky top-0 bg-zinc-950 border-b border-zinc-800 px-4 py-3 z-30">
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <button
              ...
              className="text-amber-400 font-semibold text-sm truncate max-w-[180px] hover:underline"
            >
              {store.filename}
            </button>
        ...
        {saveError && (
          <p className="text-red-400 text-xs mt-1 truncate">{saveError}</p>
        )}
      </div>
      ...
      <Modal open={filenameModalOpen} onClose={() => setFilenameModalOpen(false)} title="Renomear arquivo">
        <div className="space-y-3">
          <input
            ...
            onKeyDown={e => { if (e.key === 'Enter') { store.setFilename(editFilename); setFilenameModalOpen(false) } }}
            className="w-full bg-zinc-800 text-white rounded-lg px-3 py-2 border border-zinc-700 focus:outline-none focus:border-amber-500"
          />
          <Button
            variant="primary"
            className="w-full justify-center"
            onClick={() => { store.setFilename(editFilename); setFilenameModalOpen(false) }}
          >
            OK
          </Button>
        </div>
        ...
        {previewItem && (
          <div className="h-[75vh] -m-5 rounded-2xl overflow-hidden relative">
            {previewLoading ? (
              ...
              <SheetPage
                item={previewItem}
                chordText={previewText}
                nextTitle={
                  musicItems[musicItems.indexOf(previewItem) + 1]?.songTitle
                }
              />
            )}
            <div className="absolute bottom-4 right-4 flex gap-2">
              <Button
                ...
                onClick={async () => {
                  if (previewItem) {
                    await exportSheetPDF({ item: previewItem, chordText: previewText })
                  }
                }}
              >PDF</Button>
            </div>
